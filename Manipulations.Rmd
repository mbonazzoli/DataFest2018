...
indeedState<-indeedState%>%
  ...
  mutate(clickPerPerson = stateClicks/statePopulation) %>%
  mutate(clicksPerUnemployed = stateClicks/(statePopulation*stateUnemployment))
```
...
zipCodeInfo <- tidycensusPopulationInfo %>%
...
zipCodeInfo <- zipCodeInfo[complete.cases(zipCodeInfo),]
